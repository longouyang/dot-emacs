...
Keep vendored single-file packages on `load-path`.  The config root itself is
loaded explicitly where needed; putting `user-emacs-directory` on `load-path`
triggers an Emacs 30 warning and can shadow built-in libraries.
...
(setq insert-directory-program
      (cond ((file-executable-p "/opt/homebrew/bin/gls")
...
;; These legacy packages remain functional as Lisp/byte-code but still contain
;; compiler-only compatibility and lint issues.  Keep Emacs 30 from attempting
;; to JIT-native-compile them; native-compiler warnings remain enabled globally.
(when (native-comp-available-p)
  (require 'comp-run)
  (dolist (pattern '("/org-drill-[^/]+/"
                     "/jupyter-[^/]+/"
                     "/poly-R-[^/]+/"
                     "/polymode-[^/]+/"
                     "/quack\\.el\\'"
...
(defun my/macos-dark-appearance-p ()
  "Return non-nil when macOS is currently using dark Appearance."
  (and (eq system-type 'darwin)
       (with-temp-buffer
         (and (eq 0
                  (call-process "/usr/bin/defaults" nil t nil
                                "read" "-g" "AppleInterfaceStyle"))
              (string-match-p "\\`Dark[[:space:]]*\\'"
                              (buffer-string))))))

(defun my/solarized-theme-for-system-appearance ()
  "Return the Solarized theme matching the current system Appearance."
  ;; Preserve the old dark default on non-macOS systems.
  (if (and (eq system-type 'darwin)
           (not (my/macos-dark-appearance-p)))
      'solarized-light
    'solarized-dark))

(defun my/sync-solarized-theme-with-system-appearance ()
  "Switch Solarized themes when macOS Appearance changes."
  (interactive)
  (when (and (eq system-type 'darwin)
             (featurep 'solarized-theme))
    (let ((theme (my/solarized-theme-for-system-appearance)))
      (unless (equal custom-enabled-themes (list theme))
        (condition-case err
            (load-theme theme t)
          (error
           (message "Could not sync Solarized with macOS Appearance: %s"
                    (error-message-string err))))))))

(defvar my/macos-theme-sync-timer nil
  "Timer used to periodically sync Solarized with macOS Appearance.")

(defun my/start-macos-theme-sync ()
  "Start Solarized synchronization with macOS Appearance."
  (my/sync-solarized-theme-with-system-appearance)
  (when (timerp my/macos-theme-sync-timer)
    (cancel-timer my/macos-theme-sync-timer))
  ;; Focus changes sync immediately; this catches changes while Emacs remains
  ;; focused without polling more often than every five minutes.
  (setq my/macos-theme-sync-timer
        (run-at-time 300 300
                     #'my/sync-solarized-theme-with-system-appearance)))
...
(use-package solarized-theme
  ...
  :config
  (load-theme (my/solarized-theme-for-system-appearance) t)
  (add-hook 'after-init-hook #'my/start-macos-theme-sync)
...
(when (member "SF Mono" (font-family-list))
  ...
  ;;             (set-default-font "Noto Color Emoji")
  ;;             (set-face-attribute 'default nil :font "Noto Color Emoji" :height 120))
#+END_SRC
...
addresses menu bar flashing issues with m-x clone-indirect-buffer https://emacs.stackexchange.com/questions/14582/syntax-highlighting-fails-on-clone-indirect-buffer-other-window
#+begin_src emacs-lisp
(menu-bar-mode -1)
#+end_src
* expand-region
...
(require 'yasnippet)
(add-to-list 'yas-snippet-dirs "~/.emacs.d/snippets/ess-mode")
(yas-global-mode 1)
#+end_src
...
(use-package ess-mode
  ...
  ;; Use RStudio-style indentation.
  :custom (ess-default-style 'RStudio))
#+END_SRC
** org mode

Org Drill is installed as its own package; Org itself comes from Emacs 30.
...
;; Start previously unseen Org files in overview mode (top-level headings
;; only).  For files visited before, restore their exact folding regions.
(setq org-startup-folded 'fold)

(defvar my/org-fold-state-file
  (expand-file-name "org-fold-state.el" user-emacs-directory)
  "File used to persist Org folding regions between visits.")

(defvar my/org-fold-state-data nil
  "Alist mapping local Org file names to saved folding state.")

(defvar my/org-fold-state-loaded nil
  "Non-nil after `my/org-fold-state-data' has been loaded from disk.")

(defvar-local my/org-fold-state-content-hash nil
  "Hash of the current Org buffer before its latest unsaved edit.")

(defun my/org-fold-state-load ()
  "Load persisted Org folding state once, without evaluating its contents."
  (unless my/org-fold-state-loaded
    (setq my/org-fold-state-loaded t
          my/org-fold-state-data nil)
    (when (file-readable-p my/org-fold-state-file)
      (condition-case err
          (with-temp-buffer
            (insert-file-contents my/org-fold-state-file)
            (let ((read-eval nil))
              (setq my/org-fold-state-data (read (current-buffer))))
            (unless (listp my/org-fold-state-data)
              (error "expected an alist")))
        (error
         (setq my/org-fold-state-data nil)
         (message "Ignoring invalid Org fold state: %s"
                  (error-message-string err)))))))

(defun my/org-fold-state-write ()
  "Write `my/org-fold-state-data' to `my/org-fold-state-file'."
  (condition-case err
      (let ((print-length nil)
            (print-level nil))
        (with-temp-file my/org-fold-state-file
          (insert ";;; Machine-local Org folding state; generated automatically.\n")
          (prin1 my/org-fold-state-data (current-buffer))
          (insert "\n")))
    (error
     (message "Could not save Org fold state: %s"
              (error-message-string err)))))

(defun my/org-fold-state-key ()
  "Return a stable persistence key for the current local file."
  (when (and buffer-file-name
             (not (file-remote-p buffer-file-name)))
    (file-truename buffer-file-name)))

(defun my/org-fold-state-hash ()
  "Return a hash of all text in the current buffer."
  (save-restriction
    (widen)
    (secure-hash 'sha256 (current-buffer))))

(defun my/org-fold-state-invalidate-hash (&rest _)
  "Forget the cached content hash after a text edit."
  (setq my/org-fold-state-content-hash nil))

(defun my/org-fold-state-regions-valid-p (regions)
  "Return non-nil when REGIONS can safely be restored in this buffer."
  (and
   (listp regions)
   (catch 'invalid
...
(defun my/org-fold-state-capture ()
  "Capture this unmodified local Org buffer's current folding state.
Return non-nil when state was captured."
  (let ((key (my/org-fold-state-key)))
    (when (and key
               (derived-mode-p 'org-mode)
               (not (buffer-modified-p)))
      (require 'org-fold)
      (my/org-fold-state-load)
      (setq my/org-fold-state-content-hash
            (or my/org-fold-state-content-hash
                (my/org-fold-state-hash)))
      (let ((entry
             (list :hash my/org-fold-state-content-hash
                   :regions (org-fold-get-regions))))
        (setq my/org-fold-state-data
              (cons (cons key entry)
                    (assoc-delete-all key my/org-fold-state-data))))
      t)))

(defun my/org-fold-state-save (&rest _)
  "Capture and persist the current Org buffer's folding state."
  (when (my/org-fold-state-capture)
    (my/org-fold-state-write)))

(defun my/org-fold-state-restore ()
  "Restore folding state for the current Org file when its text matches."
  (let ((key (my/org-fold-state-key)))
    (when key
      (require 'org-fold)
      (my/org-fold-state-load)
      (setq my/org-fold-state-content-hash (my/org-fold-state-hash))
      (let* ((entry (cdr (assoc key my/org-fold-state-data)))
             (regions (and entry (plist-get entry :regions))))
        (when (and entry
                   (plist-member entry :regions)
                   (equal my/org-fold-state-content-hash
                          (plist-get entry :hash))
                   (my/org-fold-state-regions-valid-p regions))
          ;; Copy because `org-fold-regions' may destructively filter its input.
          (org-fold-regions (copy-tree regions) :override t))))))

(defun my/org-fold-state-enable ()
  "Enable per-file Org folding persistence in the current buffer."
  (my/org-fold-state-restore)
  (add-hook 'after-change-functions
            #'my/org-fold-state-invalidate-hash nil t)
  (add-hook 'after-save-hook #'my/org-fold-state-save nil t)
  (add-hook 'kill-buffer-hook #'my/org-fold-state-save nil t))

(defun my/org-fold-state-save-all ()
  "Capture all unmodified local Org buffers and persist them together."
  (let (captured)
...
(add-hook 'org-mode-hook #'my/org-fold-state-enable)
(add-hook 'kill-emacs-hook #'my/org-fold-state-save-all)
...
(require 'org-drill)
(require 'org-tempo) ;; https://emacs.stackexchange.com/questions/40571/how-to-set-a-short-cut-for-begin-src-end-src#comment72314_40576
#+END_SRC

TODO: 2019.02.18 this doesn't work perfectly.
i need to manually an org-capture first before the keyboard shortcut works.
i also don't know why org-capture-get just does the right thing -- possibly it's because i only have one capture template?

#+begin_src emacs-lisp
(setq org-capture-templates
      '(("d" "org-drill card" entry (file "~/anki.org")
         "* %?:drill:\n# created %T\n** answer\n")
...
(add-hook 'org-mode-hook (lambda () (local-set-key (kbd "s-=")
                                                   (lambda () (interactive) (org-drill-insert-card)))))
#+END_SRC
...
** Church
#+BEGIN_SRC emacs-lisp
(load (expand-file-name "quack.el" user-emacs-directory))
(load (expand-file-name "church.el" user-emacs-directory))
...
(defun LaTeX-mode-keys ()
  ...
  (local-set-key (kbd "s-t") (lambda ()  (interactive) (TeX-font nil ?\C-t)))
  )
...
(use-package markdown-mode
  ...
  (setq markdown-css-paths '("file:///Users/long/.emacs.d/markdown.css"))
  )
#+end_src
...
(use-package ace-jump-mode
  ...
  (key-chord-define-global "kq" 'ace-jump-mode-pop-mark)
...
(use-package origami
  ...
  (set-face-background 'origami-fold-replacement-face "#deb887")
  (set-face-foreground 'origami-fold-replacement-face "#000000")
...
(set-face-background 'error "#cb4b16")
(set-face-foreground 'error "#ffffff")

;; HT https://github.com/bbatsov/solarized-emacs/pull/198
;; (solarized-with-color-variables 'light
;;   (set-face-attribute 'org-level-1 nil :foreground `,blue :slant 'italic)
;;   (set-face-attribute 'org-level-2 nil :foreground `,magenta :slant 'italic)
;;   (set-face-attribute 'org-level-3 nil :foreground `,yellow :slant 'italic)
;;   (set-face-attribute 'org-level-4 nil :foreground `,cyan :slant 'italic)
;;   (set-face-attribute 'org-level-5 nil :foreground `,base01 :slant 'italic)
;;   (set-face-attribute 'org-level-6 nil :foreground `,orange :slant 'italic)
;;   (set-face-attribute 'org-level-7 nil :foreground `,violet :slant 'italic)
;;   (set-face-attribute 'org-level-8 nil :foreground `,green :slant 'italic)
;; )
...
;; from chatgpt
(defun my/org-block-faces-solarized ()
  "Force Org src block faces to our Solarized-derived colors."
  (when (featurep 'solarized-theme)
    (require 'solarized-palettes)
    ;; Pick palette based on whether a light theme seems enabled.
    (let* ((enabled custom-enabled-themes)
           (is-light (seq-some (lambda (th)
                                (string-match-p "light" (symbol-name th)))
                              enabled))
           (palette (if is-light
                        solarized-light-color-palette-alist
                      solarized-dark-color-palette-alist))
           (color (lambda (name) (alist-get name palette)))
           (block-background
            (funcall color (if is-light 'base2 'base02))))
      ;; Headings
      (set-face-attribute 'org-level-1 nil :foreground (funcall color 'blue)    :slant 'italic)
      (set-face-attribute 'org-level-2 nil :foreground (funcall color 'magenta) :slant 'italic)
      (set-face-attribute 'org-level-3 nil :foreground (funcall color 'yellow)  :slant 'italic)
      (set-face-attribute 'org-level-4 nil :foreground (funcall color 'cyan)    :slant 'italic)
      (set-face-attribute 'org-level-5 nil :foreground (funcall color 'base01)  :slant 'italic)
      (set-face-attribute 'org-level-6 nil :foreground (funcall color 'orange)  :slant 'italic)
      (set-face-attribute 'org-level-7 nil :foreground (funcall color 'violet)  :slant 'italic)
      (set-face-attribute 'org-level-8 nil :foreground (funcall color 'green)   :slant 'italic)

      ;; Blocks + begin/end lines
      (set-face-attribute 'org-block nil :background block-background :extend t)
      (set-face-attribute 'org-block-begin-line nil :background block-background :extend t)
      (set-face-attribute 'org-block-end-line nil :background block-background :extend t))))

;; Replace your old defadvice with modern advice that also reapplies faces.
(advice-add
 'load-theme :around
 (lambda (orig theme &optional no-confirm no-enable)
   (mapc #'disable-theme custom-enabled-themes)
   (prog1 (funcall orig theme no-confirm no-enable)
     ;; Only apply if the theme got enabled
     (unless no-enable
       (my/org-block-faces-solarized)))))

;; Also apply once after init (covers first startup load)
(add-hook 'after-init-hook #'my/org-block-faces-solarized)
#+END_SRC
...
#+begin_example
(setenv "SSH_AUTH_SOCK" "Users/long/Library/Containers/com.maxgoedjen.Secretive.SecretAgent/Data/socket.ssh")
#+end_example
